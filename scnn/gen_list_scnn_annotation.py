...
def generate_data(path):
    subtitles = [x for x in os.listdir(path) if (x.endswith('aqt')) and not (x.endswith('auto.aqt')) and not (x.endswith('_pad.aqt'))] 
    for aqt_file in subtitles:
        ...
        while current_line < len(subtitle):
            ...
            with open("/media/data/mtriet/dataset/annotation/%s/%s.txt" % (sys.argv[1], current_class), 'a') as f:
...
if __name__ == '__main__':
  if len(sys.argv) < 2:
    ...
    sys.exit(1)
  PATH = "/media/data/mtriet/raw_video/%s/val/" % sys.argv[1]
  with open('../%s_origin_classes.json' % sys.argv[1], 'r') as f:
    classes = json.load(f)
    count = [0] * len(classes)
